...
[role="lead"]
您可以从 NetApp Console 中的 Keystone 仪表板查看和创建 Keystone 订阅的警报监视器。仪表板显示系统生成的和用户定义的容量使用和订阅到期监视器。系统生成的监视器会自动为所有订阅创建，并带有预定义的阈值。您还可以创建用户定义的监视器，以设置自定义阈值、选择警报严重性以及为通知添加电子邮件收件人。您可以通过将监视器添加到未覆盖的订阅或解决未使用的监视器来弥补覆盖率差距。此功能在 Digital Advisor 中不可用。
...
+
您可以查看跨 Keystone 订阅的警报监视器列表，包括监视器名称、触发通知的监视器条件、严重性级别、订阅号、性能服务级别和状态。您可以检查*创建者*列，以查看警报监视器是系统生成的还是用户定义的。

+
Keystone 会自动为您的所有订阅创建系统生成的监视器。这些监视器无法删除。编辑仅限于修改容量使用监视器的订阅编号和性能服务级别，以及修改订阅到期监视器的订阅编号。

+
** *订阅到期监视器*：在订阅到期日期前五个阈值（180、90、60、30 和 7 天）触发警报。随着到期日期的临近，严重程度从信息升级为关键。
** *容量使用率监视器*：警报基于您的订阅的承诺突发百分比，而不是固定值。例如，如果您的承诺突发为 40%，则当容量利用率达到承诺容量的 140% 时，会触发关键警报。
+
