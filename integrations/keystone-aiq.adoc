...
NOTE: Keystone 订阅中的容量消耗在仪表板和报告中以 TiBs 显示，并四舍五入到小数点后两位。如果使用量小于 0.01 TiB，则该值显示为 0 或 No Usage。这些屏幕上的数据以 UTC 时间（服务器时区）显示。当您输入查询日期时，它将自动被视为 UTC 时间。要了解有关使用情况指标的更多信息，请参阅 link:../concepts/metrics.html#metrics-measurement["衡量指标"]。有关 Keystone 中使用的不同容量的信息，请参阅 link:../concepts/supported-storage-capacity.html["Keystone 中的存储容量类型"]。
