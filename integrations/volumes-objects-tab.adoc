...
+
您可以使用 *Hide/Show Columns* 来添加或删除表格中显示的列。默认情况下，该表显示您以前保存的列首选项。新添加的列，如 *Department* 或 *Total Footprint* 列，默认情况下是隐藏的，应手动选择以显示在表中。您可以选择或取消选择任何列，您的首选项将被保存以供以后使用。下载 CSV 报告时，无论您的显示首选项如何，所有可用列都将包含在导出中。
