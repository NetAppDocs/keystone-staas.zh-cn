...
+
您可以看到关于群集中的卷的详细信息，包括卷名、节点序列号、群集名称、主机名和性能服务级别。您还可以监控调配的容量、逻辑和物理利用率、冷数据和热数据。

+

NOTE: *Cold tier %* 和 *Hot tier %* 列显示每个层的数据百分比，分别计算为（Cold data / Total footprint）× 100 和（Hot data / Total footprint）× 100。
