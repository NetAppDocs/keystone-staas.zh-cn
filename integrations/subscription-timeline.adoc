...
. 选择“*订阅时间表*”选项卡。
+
image:console-subscription-timeline-graph.png["屏幕截图以折线图形式显示订阅时间线以及重要事件"]

+
默认情况下，订阅时间轴显示为折线图，图标标记重要事件。当同一天发生多个不同类型的订阅更新时，图表将显示一个数字而不是单独的图标。将鼠标悬停在数字上可查看当天发生的所有更新类型，或将鼠标悬停在任何事件图标上可查看日期和承诺容量更改等详细信息。要在表格中查看这些事件，请从 *View as* 下拉列表中选择 *Table* 选项。要查看其他订阅时间表，请从 *Tracking ID* 下拉列表中选择或搜索。

+
image:console-subscription-timeline.png["以表格图表形式显示订阅时间线以及重要事件的屏幕截图"]
