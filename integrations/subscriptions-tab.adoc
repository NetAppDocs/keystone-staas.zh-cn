...
+
image:console-subscription.png["查看订阅标签详情"]
...
+
** *订阅号*：由 NetApp 分配的 Keystone 订阅的订阅号。带有 image:metrocluster-icon.png["MetroCluster 订阅符号"] 图标的订阅是 MetroCluster 订阅。要了解有关 MetroCluster 订阅的更多信息，请参阅 link:../integrations/adp-tab.html["查看 Keystone MetroCluster 订阅的消耗量和运行状况"]。
** *跟踪ID*：激活订阅时分配的跟踪ID。这是每个订阅和站点的唯一ID、用于跟踪订阅。
** *客户名称*：与 Keystone 订阅相关的客户的名称。
** *链接订阅*：如果您有任何辅助订阅链接到您的主要订阅，此列将列出主要订阅的链接订阅编号。默认情况下，此列是隐藏的，仅当链接的订阅可用时才可见。
** *Keystone 版本*：用于订阅的 Keystone 服务版本。性能服务级别的费率计划规则可能因订阅版本而异。要了解有关版本 v1 的更多信息，请参阅 https://docs.netapp.com/us-en/keystone/index.html["Keystone 订阅服务 | 版本 1 文档"^]。默认情况下，此列是隐藏的，可以使用列选择器image:column-selector.png["列选择器图标"]图标启用。
** *使用类型*：您可能已经订阅了多个 Keystone 版本。性能服务级别的费率计划规则可能因订阅版本而异。通过查看此列中的值，您可以知道使用类型是按预配、物理还是逻辑使用计费。要了解有关版本 1 的更多信息，请参阅 https://docs.netapp.com/us-en/keystone/index.html["Keystone 订阅服务 | 版本 1 文档"^]。
** *计费周期*：订阅的发票周期。
** *性能服务级别*：与订阅相关的费率计划数量，每个费率计划对应特定的性能服务级别和承诺容量。
** *最高容量*：订阅内消耗的最大容量。
** *订阅到期天数*：在到期日期旁边显示一个图标，表明订阅即将到期以及是否会自动续订。此状态为只读，反映了您的合同条款。要查看合同详细信息，请参阅 link:../integrations/subscription-timeline.html["查看 Keystone 订阅的时间线"]。
+
当自动续订处于关闭状态时，图标颜色表示接近到期时间：

+
*** *绿色*：距离过期还有 120 天以上。
*** *橙色*：距到期还有 91 至 120 天。
*** *红色*：距到期不足 90 天。为避免服务中断，请向 NetApp Keystone 支持部门提交续订申请。
*** *灰色*：订阅已过期。
+
对于自动续订的订阅，将显示一个自动续订图标，表明订阅将在当前期限结束时自动续订。

+

NOTE: 您可以单击表标题中的向下箭头 image:down-arrow.png["展开图标"] 以展开所有订阅，并同时查看每个订阅的性能服务级别信息。这显示了详细的容量信息，包括当前使用量、承诺容量、配置容量、突发容量和所有订阅的可用容量。或者，您可以单击单个订阅行中 *Expiration date* 列旁边的向下箭头，一次展开一个。
...
** *Linked Subscriptions*：如果您有任何辅助订阅链接到您的主要订阅，此列将列出主要订阅的链接订阅编号。
