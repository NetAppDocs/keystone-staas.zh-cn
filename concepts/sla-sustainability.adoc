...
如果瓦特/ TiB消耗量超过或不符合SLA中为某个服务级别指定的标准、NetApp将针对计费期间报告的差异为所有符合条件的订阅提供服务抵免付款。如果未满足服务级别SLA指标、请在服务月结束前一周内与NetApp全球服务支持中心(GSSC)联系在确定违反服务级别时、将根据服务级别的已提交容量计算抵免额、并在下个月的发票中记入贷方。
