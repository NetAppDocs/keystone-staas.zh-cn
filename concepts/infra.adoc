...
* Keystone Data Collector：Keystone收集器为Keystone客户提供计费服务。此应用程序与Active IQ Unified Manager 捆绑在一起。它每五分钟定期从ONTAP 和StorageGRID 控制器收集一次数据。处理数据、并通过用于生成计费数据的AutoSupport 机制将元数据发送到集中式Active IQ 数据湖。Active IQ 数据湖处理数据以进行计费、并将其发送给Zuora进行计费。您可以通过Digital Advisor客户门户登录并查看Keystone订阅的订阅和消费详细信息。


image:mgmt-stack.png[""]

有关详细信息，请参见 link:../integrations/keystone-aiq.html["Keystone和Digital Advisor"]。
