...
[role="lead"]
针对文件和块存储的 Keystone STaaS 标准服务包括分层功能，可识别使用频率较低的数据，并将其分层到 Keystone STaaS 支持的 NetApp 冷存储。如果要将冷数据分层到任何 Keystone STaaS 支持的非 NetApp 存储，您可以将数据分层用作附加服务。此附加服务仅在统一数据存储类型上受支持。
...
NOTE: 仅当数据分层到任何非 NetApp 存储（如 Amazon Web Services (AWS) S3、Azure Blob、Google Cloud Platform (GCP) 和其他 Keystone STaaS 支持的、S3 兼容的第三方对象存储）时，才需要分层附加服务。此服务仅适用于统一数据存储类型。
